...
class Head (object):
    def __init__(self, video_src,detector,predictor):
        
        self.vs = VideoStream(video_src).start()
        time.sleep(2.0)
    ...
    def run(self):
        ...
        # grayscale
            frame = self.vs.read()
            frame = imutils.resize(frame, self.frame_width, self.frame_height)
            ...
            for rect in rects:
                    ...
                    Euler = rotation_vector

                    ## Unity Transfer

                    data = str(Euler[0]) + str(",") + str(Euler[1]) + str(",") + str(Euler[2])
                    ...
                    # Use this to draw a line sticking out of the nose_end_point2D
...
if __name__ == '__main__':
    ...
    ## Connection between python and Unity through socket connection
    ...
    video_src = 0
    Head(video_src,detector,predictor).run()