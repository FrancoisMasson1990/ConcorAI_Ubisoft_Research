...
namespace UPyPlot {
	public class UPyPlotController : MonoBehaviour {
        ...
        [SerializeField] private string plotFileName;
		...
		void Awake () 
		{
            ...
            string plotDir = "/Users/Francois/ConcordAI/Head_Analysis/Data_Save/"; // The directory to create the plot data file in.
		...
		void CheckProbes () 
		{
			if (probes.Count > 0) {
				...
				for (int i = 0; i < probes.Count; i++) {
                    ...
                    line += ((float)fieldInfo.GetValue(mono)).ToString("G9");
                    if (i < probes.Count - 1) {
				...
				if (currentSample >= maxSamples) { // Handle rolling the file when max number of lines has been reached.
					...
					lines.RemoveRange (1, range); // Dont modify the first two lines (meta data, header data) but remove all lines for range after them.
		...
		private void CacheProbes() 
		{
			...
			*/
            ...
            object[] obj = GameObject.FindObjectsOfType(typeof (GameObject)); // All gameobjects in the scene.
		...
		private void CreateFileHeader() 
		{
			...
			string line = "rotation.x,rotation.y,rotation.z,timestamp"; // The string that will be written into the plot data file.
			File.WriteAllText(absoluteName,line + '\n');
		}
	}
}